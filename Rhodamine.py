...
# 5개의 Noise ASC 파일 경로
noise_paths = ["NOISEOF567AREA/NOISEOFRODAMINE1.asc",
               "NOISEOF567AREA/NOISEOFRODAMINE2.asc",
               "NOISEOF567AREA/NOISEOFRODAMINE3.asc",
               "NOISEOF567AREA/NOISEOFRODAMINE4.asc",
               "NOISEOF567AREA/NOISEOFRODAMINE5.asc"]
...
for index, row in df.iterrows():
        ...
        energy_data.append(1239.8/float(row[0]))
...
# 5개의 Rhodamine 파일 경로
data_paths = ["RODAMINE/RODAMINE1.asc",
              "RODAMINE/RODAMINE2.asc",
              "RODAMINE/RODAMINE3.asc",
              "RODAMINE/RODAMINE4.asc",
              "RODAMINE/RODAMINE5.asc"]
...
# 초기 추정값 설정
initial_guess = [1600, np.average(x_data), 10]  # amp, cen, wid
...
print("Amplitude, Center, and Width is", amp, cen, wid)
...
'''plt.figure()
plt.plot(energy_data, y_data, color = 'black', label = 'Original data')
plt.xlabel('Laser energy[eV]')
plt.ylabel('Photon counts')
plt.legend()
plt.show()
 # 그래프 eV 단위로 그리기'''